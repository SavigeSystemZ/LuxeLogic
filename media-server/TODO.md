...
- [ ] HIGH: Refine `PRODUCT_BRIEF.md` for media-server so the product frame and first build shape are explicit
...
- [ ] HIGH: Establish the first validated baseline for media-server

## Immediate Queue

Use short, reviewable tasks with priority signals.

- [ ] MEDIUM: Finish onboarding and confirm the first working validation path for media-server
